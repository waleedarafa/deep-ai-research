...
describe("pulseSubagents shape", () => {
  ...
  it("requires Exa tools on paper-curator", () => {
  ...
  it("hn-curator uses hn-trending tool", () => {
    expect(pulseSubagents["hn-curator"].tools).toEqual(
      expect.arrayContaining(["mcp__hn-trending__list_trending"])
    );
  });
});
