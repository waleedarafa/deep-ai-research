...
**Goal:** Return up to 8 tweets with the strongest signal (engagement, not just announcements).

**Tool:** mcp__x-trending__list_recent_tweets (already merges curated-account whitelist + open-discovery search across the full 100-tweet API pool, applies an engagement threshold, returns ranked-by-engagement tweets; returns [] when X_BEARER_TOKEN is unset)

**Procedure:**
1. Call mcp__x-trending__list_recent_tweets with since_hours=24, limit=8, min_engagement=100.
